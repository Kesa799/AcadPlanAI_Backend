...
class AIService:
    ...
    def generate_cdp(self, syllabus_text: str, additional_prompt: Optional[str] = None, file_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.mock_mode or not syllabus_text:
            ...
            cdp_data = self._merge_with_defaults(extracted, syllabus_text)
        else:
            try:
                # Use OpenAI directly
                ...
                if json_match:
                    ...
                    cdp_data = self._merge_with_defaults(data, syllabus_text)
                else:
        ...
        if file_path:
            cdp_data = self._post_process_co_po_mappings(cdp_data, file_path)
        return cdp_data

    def generate_local_cdp(self, syllabus_text: str, file_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _post_process_co_po_mappings(self, cdp_data: Dict[str, Any], file_path: str) -> Dict[str, Any]:
        """Post-process CDP data to extract and map CO-PO relationships from document tables."""
        try:
            from app.parsers import DocumentParser
            parsed = DocumentParser.parse(file_path)
            tables = parsed.get("tables", [])
        except Exception as e:
            print(f"Table extraction failed during post-processing: {str(e)}")
            return cdp_data
            ...
            for r_idx, row in enumerate(table[:3]):
                ...
                for c_idx, cell in enumerate(row):
                    if cell:
                        ...
                        match = re.search(r'\b(PO\d+|PSO\d+)\b', cell_clean)
                        if match:
                            po_cols[c_idx] = match.group(1)

                if len(po_cols) >= 3:
                    po_header_row_idx = r_idx
                    col_to_po = po_cols
                    break

            if po_header_row_idx == -1:
                continue

            for row in table[po_header_row_idx + 1:]:
                if not row:
                    continue
                first_cell = str(row[0]).strip() if row[0] is not None else ""
                co_match = re.search(r'\b(CO\d+)\b', first_cell, re.IGNORECASE)
                if not co_match:
                    continue
                ...
                for c_idx, cell in enumerate(row):
                    if c_idx in col_to_po:
                        po_id = col_to_po[c_idx]
                        if cell is not None:
                            cell_clean = str(cell).strip()
                            val_match = re.search(r'^\d+(\.\d+)?$', cell_clean)
                            if val_match:
                                score = float(cell_clean)
                                row_mappings[po_id] = score
                                mapped_po_ids.add(po_id)

                if row_mappings:
                    co_po_affinity_map[co_id] = row_mappings

        if co_po_affinity_map:
            cdp_data["co_po_affinity_map"] = co_po_affinity_map

            # Update mapped_pos for each Course Outcome
            updated_cos = []
            for co in cdp_data.get("course_outcomes", []):
                co_id = co.get("id", "").upper()
                mapped = sorted(
                    list(co_po_affinity_map.get(co_id, {}).keys()),
                    key=lambda x: (0 if x.startswith('PO') else 1, int(re.search(r'\d+', x).group()) if re.search(r'\d+', x) else x)
                )
                co["mapped_pos"] = mapped
                updated_cos.append(co)
            cdp_data["course_outcomes"] = updated_cos

            # Standard Program Outcomes definitions for engineering
            ...
            program_outcomes = []
            sorted_po_ids = sorted(
                list(mapped_po_ids),
                key=lambda x: (0 if x.startswith('PO') else 1, int(re.search(r'\d+', x).group()) if re.search(r'\d+', x) else x)
            )
            for po_id in sorted_po_ids:
                desc = STANDARD_PROGRAM_OUTCOMES.get(po_id)
                if not desc:
                    if po_id.startswith("PSO"):
                        desc = f"Program Specific Outcome {po_id[3:]}"
                    else:
                        desc = f"Program Outcome {po_id[2:]}"
                program_outcomes.append({
                    "id": po_id,
                    "description": desc
                })
            cdp_data["program_outcomes"] = program_outcomes
